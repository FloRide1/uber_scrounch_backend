...
use crate::{state::PoolType, models::product_model::ProductModel};
...
pub async fn get_product_ids(State(pool): State<PoolType>) -> Json<Vec<i32>> {
    let res = pool.get().await.unwrap().interact(move |conn| {
        ProductModel::list(conn).unwrap_or(vec![])
    }).await.unwrap();

    Json(res)
}

pub async fn get_product(Path(id): Path<i32>, State(pool): State<PoolType>) -> Result<ProductResponse, impl IntoResponse> {
    let res = pool.get().await.unwrap().interact(move |conn| {
        ProductModel::get(conn, id)
    }).await.unwrap();

    match res {
        Ok(res) => Ok(res.into()),
        Err(err) => match err {
            Error::NotFound => Err((StatusCode::NOT_FOUND, format!("The product with id: \"{id}\" doesn't exist"))),
...
pub async fn get_all_product_list(State(pool): State<PoolType>) -> Json<Vec<ProductResponse>> {
    let products = pool.get().await.unwrap().interact(move |conn| {
